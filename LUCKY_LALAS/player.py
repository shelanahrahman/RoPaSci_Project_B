...
def coordinatesInBoundary(coordinatetuple):
  """given coordinates, if it's in the boundaries of board, it returns true. Otherwise, returns False"""
...
def getThrows(playerDict, enemyDict, playerTeam, throw_dist, firstturn=False, spec= False):
    """returns a list of all possible throws for playerDict
       firstturn means that the function should return the first move generated directly rather than
       appending to move_lst
       spec is a list of unique token types which indicates the token types needed to be thrown in order
       to beat the enemy tokens"""
    # Format of move_lst=[throwmove,..., throwmove] where throwmove= ["Throw", token_type, coordinate]
    move_lst=[]
    if spec:
        token_type_lst= spec
    else:
...
def check_instant_kill(enemydict, playerthrowdist, playerteam):
    """ returns an action or 0 depending on if it is possible to throw a token and beat the opponent's token"""
    ...
    #Find possible direct throw kills
    ...
    #if there is no direct throw kills, return 0
    return 0
...
def getSlides(cell, tokentype, playerDict, enemyDict, is_swing=0, og_coordinate=False):
    ...
    dont_add=0
    ## if there is an original coordinate, remove og coordinate and the slide coordinates from og as swing options
    if og_coordinate:
        ...
        r_directions= r_lst
        q_directions= q_lst
    # for every possible slide/swing move, check if it is a valid slide/swing move
...
def validMove(unsorted_move_dict, playerDict, enemyDict, tok_type, coordinate_a, coordinate_b):
    """ This function assures that move is valid and adds a direction token to the move_lst
...
def create_teamtokens_on_board(team):
    """ returns a list of unique token types given a team dictionary"""
...
def hex_dist_val(p,q):
    """returns the distance between two hexes give points p (r,q) and q (r,q)"""
    curr_sum=0
    #if going left
    if q[1]<p[1]:
        dist1= q[0]-p[0]
...
def find_team_dist_value(team, opposing_team):
    """ find the transformed value of the sum of all the distances between team token and its specific target"""
 ...
 #       print("team dictionary in find_team_dist_value", team)
        ...
        for o_coordinate, o_toktypes in opposing_team.items():
            ...
            # if t_toktypes win, the opposing token is a target to be calculated
...
def find_team_sufficiency_value(team, opposing_team):
    ...
    #returns a value based on how many of team's tokens can beat opposing team's
    #e.g. if team1 has two rocks and one paper and team2 has 2 scissors and 2 papers, function will return 0.5 as it can only
    #beat 50% of the opposing_team with the current options of token types"""
    team_types=create_teamtokens_on_board(team)
    opposing_team_dict={}
...
def move_against(playerDict, enemyDict, playerTeam, player_throws_did, player_throw_distance):
    ...
    listofpossibledicts= [generated_dict, move that caused the generated dict, board score of how good it is]"""
    listofpossibledicts = []
  ...
  #  print("list of possible dicts", listofpossibledicts)
    ...
    return bestPossibleBoard_and_move


def playerisindanger(playerDict, enemyDict):
    """returns percentage of enemies (out of total number of enemy tokens) that are immediately next to a player's token"""
    dangerous_enemies=0
    total_enemies= sum(len(values) for values in enemyDict.values())
    for player_coordinate, tokentypes in playerDict.items():
        tokentype=tokentypes[0]
        for enemy_coordinate, enemytokentypes in enemyDict.items():
            enemytokentype= enemytokentypes[0]
            if hex_dist_val(player_coordinate, enemy_coordinate)==1 and rps_fight(tokentype, enemytokentype)==-1:
                dangerous_enemies= dangerous_enemies+1
    return dangerous_enemies/total_enemies

def evaluate(playerDict, enemyDict): 
  #provides a value to judge how good the board state- positive values favours playerDict and negative values favours enemyDict
    ...
    if playereats > enemyeats:
        immediate_danger= playerisindanger(playerDict, enemyDict)
        score = score + (1-immediate_danger) * 100
    #player loses and gets eaten
    elif enemyeats > playereats:
        immediate_danger= playerisindanger(enemyDict, playerDict)
        score = score - (1-immediate_danger) * 100
...
class Player:
    ...
    def action(self):
        ...
        #try to throw and beat an enemy token
        ...
        if (len(self.selfdict)==0 or find_team_sufficiency_value(self.selfdict, self.enemydict)<1) and self.self_throws_did<9:
            ...
            cell= list(self.selfdict.keys())[0]
            tokenType=  self.selfdict.get(cell)[0]
  ...
  #      print("player predicting...")
        ...
        player_choice = move_against(self.selfdict, enemypredicted_dict, self.team, self.self_throws_did, self.self_throw_distance)

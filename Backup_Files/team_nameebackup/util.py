...
def InputDictConversion(inputjsonfile, section):
    ## given a json file, and the section of the json file you want to convert to a dictionary,
   ## this function turns the section to a dictionary where the key is a tuple
   ## for the (row,column), and the value is the type of token)"""

    outputDict = {}
    
    for tokeninfo in inputjsonfile[section]:
        row = tokeninfo[1]
        column = tokeninfo[2]
        tokentype = tokeninfo[0]
        inputkeytuple = (row, column)
        ## tokentype changed to list so tokens of same type and team can sit on top of one cell
        outputDict[inputkeytuple] = [tokentype]
    
    return outputDict
    
    
def MakeBigDict(CollectingDict, AddingOnDict):
  
  ## adds a dictionary to another dictionary
    for key in AddingOnDict:
        CollectingDict[key] = AddingOnDict[key]
        
    return
    
def updateBigBoardDict(upperdict, lowerdict, blockdict):
  
  ### updates the big dictionary of the entire board, given three dictionaries
  
  BigBoardDict = {}
  
  MakeBigDict(BigBoardDict, upperdict)
  MakeBigDict(BigBoardDict, lowerdict)
  MakeBigDict(BigBoardDict, blockdict)

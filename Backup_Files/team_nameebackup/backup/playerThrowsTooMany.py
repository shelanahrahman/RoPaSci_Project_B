...
from team_namee.util import print_board, print_slide, print_swing, InputDictConversion, MakeBigDict, updateBigBoardDict
...
#returns a list of best possible boards+moves against the current state of the other player

def move_against(playerDict, enemyDict, playerTeam, player_throws_did, player_throw_distance):
    
    #stores, [generated_dict, move that caused the generated dict, board score of how good it is]
    listofpossibledicts = []
    ...
    for cell, tokenTypes in playerDict.items():
        for tokenType in tokenTypes:
          ...
          #generate throws only when not swings and slides possible
        #    if player_throws_did<9 and ( (len(slidesTemporary)==0 and len(swingsTemporary)==0) 
        #                            or find_team_sufficiency_value(playerDict, enemyDict)==0 ):
...
#              print(playerdictcopy)
    ...
    for potential_state in listofpossibledicts:
       ...
       #     print("appeneded cuz it's good")
       #     currently_best_score = score
       #     betterPossibleBoards_and_move.append(potential_state)
    
  #  print("list of possible dicts", listofpossibledicts)
         #[dict, move, score] 
         
    #sort better boards to find the best boards
    
    
    #sort by reverse, so best scoring boards are at the front
    listofpossibledicts.sort(key=lambda x: float(x[2]), reverse = True )
    
    #print("listofpossibledicts sorted", listofpossibledicts)
    
    bestPossibleBoards_and_move = []
    
    maxscore = float('-inf') 
    
    #pick the best scoring board at the front to have the best score
    if listofpossibledicts != []:
    
        maxscore = listofpossibledicts[0][2]
        
        #only one move, then it's the best move
        if len(listofpossibledicts) == 1:
            bestPossibleBoards_and_move = listofpossibledicts
        
        else:
            #keep only boards with scores equal to that best score
            for indexmaxslicer in range(0,len(listofpossibledicts)):
                boardscore = listofpossibledicts[indexmaxslicer][2]
                if boardscore < maxscore:
                    bestPossibleBoards_and_move = listofpossibledicts[0:indexmaxslicer]
                    break
            #if they all have equal score        
            bestPossibleBoards_and_move = listofpossibledicts
            
                    
    
   # print()
    print("best possible boards", bestPossibleBoards_and_move)   
    return bestPossibleBoards_and_move
...
class Player:
    ...
    def action(self):
       ...
       # print(self.enemydict)
        ...
        if self.selfdict == {} and self.self_throws_did < 9:
   ...
   #     print()
        ...
        enemypredictions_list = move_against(self.enemydict, self.selfdict, self.enemyteam, self.enemy_throws_did, self.enemy_throw_distance)
        
       
        #then based on what we predicted the enemy could do, we make our move?
        
        print("enemy prediction is...")
        print(enemypredictions_list)
        print()
  
        #if the best enemy predictions is blank, 
        if enemypredictions_list == []:
            
            #try predict against blank enemy? and generate best moves against it
            enemypredicted_dict = {}
            
            player_choice_list = move_against(self.selfdict, enemypredicted_dict, self.team, self.self_throws_did, self.self_throw_distance)
            
            #pick first best move available
            print("player_choice_list against blank", player_choice_list)
            player_choice = player_choice_list[0]
            
        else:
        
            #for every boardtate_listing in the  best enemy predictions,
            
            #a boardstate listing will look like
            #[dict, move, score]             
            
            player_choice_list_sum = []
            
            for boardstate_listing in enemypredictions_list:
                
                
                enemypredicted_dict = boardstate_listing[0]
                print("enemypredicted_dict", enemypredicted_dict)
                print()
                #predict against that predicted boardstate
                player_choice_list = move_against(self.selfdict, enemypredicted_dict, self.team, self.self_throws_did, self.self_throw_distance)
                
                #that returned a list of boardstate_listings
                #print("player_choice_list", player_choice_list)
                print()
                #add the player predictions to a list
                for boardstate_listing in player_choice_list:
                    player_choice_list_sum.append(boardstate_listing)
            
           # print()
           # print("player_choice_list_sum", player_choice_list_sum)
                
            #now that you have generated all the best player choices against the best enemy predictions
            # it is time to sort the best player choices according to the score
            player_choice_list_sum.sort(key=lambda x: float(x[2]), reverse = True )
            print()
            print("player_choice_list_sum sorted", player_choice_list_sum)
            
            #and find the best of the best
            
            
            #every player choice is also a boardstate listing in format
            #[dict, move, score]
            player_choice = player_choice_list_sum[0]

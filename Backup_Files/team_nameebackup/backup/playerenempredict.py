...
def move_against(playerDict, enemyDict, playerTeam, player_throws_did, player_throw_distance):
  ...
  #  print("list of possible dicts", listofpossibledicts)
    ...
    betterPossibleBoards_and_move.sort(key=lambda x: float(x[2]), reverse = True )
    
    #print("betterPossibleBoards_and_move", betterPossibleBoards_and_move)
    ...
    if betterPossibleBoards_and_move != []:
    
        maxscore = betterPossibleBoards_and_move[0][2]
        
        #keep only boards with scores equal to that best score
        for indexmaxslicer in range(0,len(betterPossibleBoards_and_move)):
            boardscore = betterPossibleBoards_and_move[indexmaxslicer][2]
            if boardscore < maxscore:
                bestPossibleBoards_and_move = betterPossibleBoards_and_move[0:indexmaxslicer]
                break

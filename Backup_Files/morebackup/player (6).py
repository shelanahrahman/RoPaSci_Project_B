        ...
        for cell, tokenTypes in playerDict.items():
            for tokenType in tokenTypes:
              ...
              #generate throws only when not swings and slides possible
                if playerMaxThrow<9:
                    if (len(slidesTemporary)==0 and len(swingsTemporary)==0) or find_team_sufficiency_value(playerDict, enemyDict)==0:
       ...
       #             print(playerdictcopy)
        ...
        for cell, tokenTypes in enemyDict.items():
            for tokenType in tokenTypes:
                ...
                if enemyMaxThrow<9:
                    if (len(slidesTemporary)==0 and len(swingsTemporary)==0) or find_team_sufficiency_value(enemyDict, playerDict)==0:
...
def evaluation(playerDict, enemyDict, depth, maxdepth, 
    initialplayereaten, initialenemyeaten, currentplayereaten, currentenemyeaten, ismax=True): 
   ...
   #         print(tokens_diff_usefulness)
            print("TOKEN_DIFF_USEFULNESS IS OVER 0 when tokdiff is ---------------------------------------",tokens_diff)
            exit()
        ...
        return sufficiency_diff*70+tokens_diff_usefulness*30
        ...
        for o_coordinate, o_toktypes in opposing_team.items():
            ...
            if rps_fight(t_toktypes, o_toktypes)==1:
                ...
                if curr_dist< closest_dist:
                    ...
                    curr_closest_o_coordinate= o_coordinate
        closest_o_coordinate.append(curr_closest_o_coordinate)
    ...
    def action(self):
        ...
        for cell, tokenTypes in self.selfdict.items():
            for tokenType in tokenTypes:
                ...
                #generate throws only when not swings and slides possible
                if self.self_throw_distance<9:
                    if (len(slidesTemporary)==0 and len(swingsTemporary)==0) or find_team_sufficiency_value(self.selfdict, self.enemydict)==0:

...
def relocate_tokens(team_dictionary, team_action):
        ...
        if team_action[0] == "THROW":
            ...
            newcoordinate = team_action[2]
            tokenType=team_action[3]
            #slide off token from its current cell's list
            if currentcoordinate=="SLIDE":
                print(team_action)
                exit()
...
def getThrows(playerDict, enemyDict, playerTeam, throw_dist, firstturn=False):
...
def getSwings(cell, tokentype, playerDict, enemyDict):
  ...
  for iterator in range(0,len(move_lst)):
    ...
    move_lst[iterator] = ["SWING"] + move_lst[iterator]
...
def minimax(playerDict, enemyDict, playerTeam, enemyTeam, playerMaxThrow, enemyMaxThrow, depth, maxdepth,  initialplayereaten, initialenemyeaten, currentplayereaten, currentenemyeaten, current_turn, maximise=True):
    ...
    return_val= evaluation(playerDict, enemyDict, depth, maxdepth, initialplayereaten, initialenemyeaten,currentplayereaten, currentenemyeaten )
    ...
    if maximise:
        ...
        for cell, tokenTypes in playerDict.items():
            for tokenType in tokenTypes:
                ...
                throwsTemporary= getThrows(playerDict, enemyDict, playerTeam, playerMaxThrow)
                ...
                for possibleMove in possibleMovelst:
                    ...
                    return minimax(playerdictcopy, enemyDict, playerTeam, enemyTeam, playerMaxThrow, enemyMaxThrow, depth+1, maxdepth, initialplayereaten, initialenemyeaten, currentplayereaten, currentenemyeaten, current_turn, False)
        ...
        for cell, tokenTypes in enemyDict.items():
            for tokenType in tokenTypes:
                ...
                throwsTemporary= getThrows(playerDict, enemyDict, enemyTeam, enemyMaxThrow)
                possibleMovelst= slidesTemporary + swingsTemporary + throwsTemporary
                for possibleMove in possibleMovelst:
                    enemydictcopy= copy.deepcopy(enemyDict)
                    relocate_tokens(enemydictcopy, possibleMove)
                    return minimax(playerDict, enemydictcopy, playerTeam, enemyTeam, playerMaxThrow, enemyMaxThrow, depth+1, maxdepth,initialplayereaten, initialenemyeaten, currentplayereaten, currentenemyeaten, current_turn)

def find_team_sufficiency_value(team, opposing_team):
    team_types= list(set(chain(*team.values())))
    opposing_team_dict={}
...
def evaluation(playerDict, enemyDict, depth, maxdepth, initialplayereaten, initialenemyeaten, currentplayereaten, currentenemyeaten): 
    #player wins
    ...
    #reached depth limit
        ...
        tokens_diff= abs(len(playerDict.values())- len(enemyDict.values()))
        tokens_diff_usefulness=(1-tokens_diff/2)
        if (tokens_diff_usefulness>1):
            print(tokens_diff_usefulness)
            print(len(playerDict.values()), len(enemyDict.values()))
            print(-abs(len(playerDict.values())- len(enemyDict.values())))
            exit()
        # check if right tokens to win
       ...
       # print(player_sufficient_tokens*0.5+enemy_sufficient_tokens*0.5)
        return player_sufficient_tokens*40+enemy_sufficient_tokens*40+tokens_diff_usefulness*10
...
class Player:
        ...
        for cell, tokenTypes in self.selfdict.items():
            for tokenType in tokenTypes:
                ...
                for possibleMove in possibleMovelst:
                    ...
                    moveEval= minimax(selfdictcopy, self.enemydict, self.team, self.enemyteam, self.self_throw_distance, self.enemy_throw_distance, 0, 2, self.self_losses, self.enemy_losses, self.self_losses, self.enemy_losses, self.current_turn, False)
                    print(moveEval)
    ...
    def update(self, opponent_action, player_action):
        ...
        if self.team== "upper":
            if opponent_action[0] == "THROW" and opponent_action[2][0]== self.enemy_throw_distance:
              self.enemy_throw_distance = self.enemy_throw_distance - 1

            if player_action[0] == "THROW" and opponent_action[2][0]== self.self_throw_distance:
